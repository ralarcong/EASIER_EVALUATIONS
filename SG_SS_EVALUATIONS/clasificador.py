...
#from sense2vecc import sense2vecc
import pandas as pd
#postag
import os
...
class clasificador:
    ...
    #sense2vector=sense2vecc()
    #bertvector=bertclass()
    #Pyphenobj=Pyphen()
    x=0
    y=0
    ...
    def getMatrix_trainxx(self, path, trigrams, totalTris, bigrams, unigrams, totalBis, totalUnis,E2Rgram):
        ...
        for row in tsvin:
            ...
            if len(word.split()) < 2:
                ...
                sense2vector=self.sense2vector.wordvectorsense(word,sentence)
                sense2vectortemp=pd.Series(sense2vector)
                ...
                vector_fet[309:437] = (sense2vectortemp * 10).tolist()
                #vector_fet[309:609] = (wordvectortemp * 10).tolist()
                vector_fet[438] = class_word
                #vector_fet[610] = class_word
    ...
    def getMatrix_traineasier(self, path, trigrams, totalTris, bigrams, unigrams, totalBis, totalUnis,E2Rgram):
        ...
        for row in tsvin:
            ##para modelo easier
            #id = row[0]  # id del parrafo donde ocurre la palabra
            #sentence = row[2]  # oracion
            #start = row[3]  # posicion inicial (caracter)
            #word = row[5]  # palabra a clasificar
            
            #class_word = row[6]  # clase: 1 o 0
            ...
            if len(word.split()) < 2:
                ...
                #bertvector=self.bertvector.bertvectors(word,sentence)
                #bertvectortemp=pd.Series(bertvector)
                ...
                vector_fet[7] = E2R
                vector_fet[8:308] = (word2vectortemp * 10).tolist()
                #vector_fet[304:432] = (sense2vectortemp * 10).tolist()
                #vector_fet[301:781] = (bertvectortemp * 10).tolist()
                #vector_fet[309:609] = (wordvectortemp * 10).tolist()
                vector_fet[309] = class_word
                #vector_fet[610] = class_word
    ...
    def getMatrix_Deploy(self, path, trigrams, totalTris, bigrams, unigrams, totalBis, totalUnis,E2Rgram):
        ...
        for row in tsvin:
            ...
            if len(word.split()) < 2:
                ...
                vector_fet[8:308] = (word2vectortemp * 10).tolist()
                #vector_fet[304:432] = (sense2vectortemp * 10).tolist()
                #vector_fet[304:784] = (bertvectortemp * 10).tolist()
                #vector_fet[309:609] = (wordvectortemp * 10).tolist()
    ...
    def SvmClassifier(self, X_train, y_train):
        ...
        classifiers = [
            #svm.SVC(kernel='linear',max_iter=1000,break_ties=True)
